...
var $author$project$Models$Article = F5(
	function (title, tags, articleText, updatedTime, uid) {
		return {articleText: articleText, tags: tags, title: title, uid: uid, updatedTime: updatedTime};
	});
...
var $author$project$Models$OtherTag = function (a) {
	return {$: 'OtherTag', a: a};
};
var $author$project$ArticlesDecoder$tagDecoder = A2($elm$json$Json$Decode$map, $author$project$Models$OtherTag, $elm$json$Json$Decode$string);
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								articles: $elm$core$List$reverse(
									A2(
			...
			case 'UrlChanged':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{url: url}),
					$author$project$Main$onUrlChange(_Utils_Tuple0));
			default:
				var req = msg.a;
				if (req.$ === 'Internal') {
					var url = req.a;
...
var $author$project$Main$articleView = F2(
	function (zone, post) {
		var tags = A2($elm$core$List$map, $author$project$Models$showTag, post.tags);
		var tagsView = A2(
			$elm$core$List$map,
			function (s) {
				return A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									'投稿日:' + A2($author$project$Main$showTime, zone, post.updatedTime))
								])),
							A2(
							$elm$html$Html$ul,
							_List_Nil,
							...
							_List_fromArray(
								[
									A2($elm_explorations$markdown$Markdown$toHtml, _List_Nil, post.articleText)
								]))
						]))
				]));
	});
								...
								[
									...
									_List_fromArray(
										[
											...
											'?post=' + $elm$core$String$fromInt(article.uid))
										]),
									_List_fromArray(
										[
											$elm$html$Html$text(
											article.title + (' ' + A2($author$project$Main$showDate, $author$project$Main$jst, article.updatedTime)))
										]))
								]));
					},
					model.articles))
					...
					[
						...
						_List_fromArray(
							[
								...
								A3($author$project$Main$articlesView, $author$project$Main$jst, model.url, model.articles)
							]))
					]))
			]),
		title: 'ブログ予定地'
